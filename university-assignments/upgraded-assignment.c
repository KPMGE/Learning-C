...
void mostraTotalCasosEntreDatas(FILE *arquivo, Data dataInicial, Data dataFinal, DadosCsv vetorCsv[]);
// função que lista em ordem alfabética as cidades com mais de N casos
void listaCasosEmOrdemAlfabetica(FILE *arquivo, int n, DadosCsv vetorCsv[]);
...
void listaTopNCidades(FILE *arquivo, int n, Data dataInicial, Data dataFinal, DadosCsv vetorCsv[]);
...
void imprimePorcentagensItem6(FILE *arquivo, char municipio[], DadosCsv vetorCsv[]);
// função que imprime os itens da questão 7
void imprimeDadosItem7(FILE *arquivo, Data dataInicial, Data dataFinal, DadosCsv vetorCsv[]);
// função que imprime a porcentagem de pessoas confirmadas internadas
void porcentagemPessoasConfirmadasInternadas(FILE *arquivo, char Municipio[], DadosCsv vetorCsv[]);
// função que imprime a porcentagem de pessoas que morreram
void porcentagemPessoasQueMorreram(FILE *arquivo, char Municipio[], DadosCsv vetorCsv[]);
// função que imprime a quantidade de pessoas internadas que morreram
void porcentagemPessoasInternadasQueMorreram(FILE *arquivo, char Municipio[], DadosCsv vetorCsv[]);
...
int calculaTotalCasos(char municipio[], DadosCsv vetorCsv[]);
// função que determina a quantidade de casos
int calculaQuantidadeDeCasosComDatas(Data dataInicial, Data dataFinal, char municipio[], DadosCsv vetorCsv[]);
...
float calculaDesvioPradraoEntreDatas(Data dataInicial, Data dataFinal, float media, DadosCsv vetorCsv[]);
// função que calcula a media, datas duas datas
float calculaMediaDeIdadeEntreDatas(Data dataInicial, Data dataFinal, DadosCsv vetorCsv[]);
// função que calcula a porcentagem de pessoas que morreram e não possuiam comorbidades
float calculaPorcentagemPessoasQueMorreramSemComorbidade(Data dataInicial, Data dataFinal, DadosCsv vetorCsv[]);
// função que realiza a leitura de uma data
void leData(Data *algumaData);
// função que passa para a próxima data
Data proximaData(Data *dataInicial);

int main(void) {
  // alocando vetor dinamicamente
  DadosCsv *vetorCsv = (DadosCsv *) malloc(TAMANHOCSV * sizeof(DadosCsv));
  ...
  leData(&dataIniTotalCasos);
  leData(&dataFimTotalCasos);
  // lendo datas e valor para top N cidades
  scanf("%d", & N);
  leData(&dataIniTopCidades);
  leData(&dataFimTopCidades);
  // lendo munícipio para o item 6
  scanf(" %[^\n]", municipioParaItem6);
  // lendo datas para media e desvio
  leData(&dataParaMediaDesvioIni);
  leData(&dataParaMediaDesvioFim);
  ...
  listaCasosEmOrdemAlfabetica(item3, NCasos, vetorCsv);
  // mostrando os casos entre data1 e data2
  mostraTotalCasosEntreDatas(item4, dataIniTotalCasos, dataFimTotalCasos, vetorCsv);
  // listando top N cidades
  listaTopNCidades(item5, N, dataIniTopCidades, dataFimTopCidades, vetorCsv);
  // mostrando porcentagens para o item 6
  imprimePorcentagensItem6(item6, municipioParaItem6, vetorCsv);
  // imprimindo valores do item 7
  imprimeDadosItem7(item7, dataParaMediaDesvioIni, dataParaMediaDesvioFim, vetorCsv);

  // liberendo memoria alocada para o vetor
  free(vetorCsv);
  // fechando arquivos
  fclose(csv);
  fclose(item3);
...
void leCsv(FILE * csv, DadosCsv vetorCsv[]) {
  ...
  for (i = 0; i < TAMANHOCSV; i++) {
    ...
    vetorCsv[i].classificacao = (vetorParaDados[0] == 'C');
    // lendo município
    ...
    vetorCsv[i].comorbidadesPacientes.comorbidadePulmao = (vetorParaDados[0] == 'S');
    fscanf(csv, "%[^,],", vetorParaDados);
    // se tiver comorbidade, variável recebe True, se não recebe False
    vetorCsv[i].comorbidadesPacientes.comorbidadeCardio = (vetorParaDados[0] == 'S');
    fscanf(csv, "%[^,],", vetorParaDados);
    // se tiver comorbidade, variável recebe True, se não recebe False
    vetorCsv[i].comorbidadesPacientes.comorbidadeRenal = (vetorParaDados[0] == 'S');
    fscanf(csv, "%[^,],", vetorParaDados);
    // se tiver comorbidade, variável recebe True, se não recebe False
    vetorCsv[i].comorbidadesPacientes.comorbidadeDiabetes = (vetorParaDados[0] == 'S');
    fscanf(csv, "%[^,],", vetorParaDados);
    // se tiver comorbidade, variável recebe True, se não recebe False
    vetorCsv[i].comorbidadesPacientes.comorbidadeTabagismo = (vetorParaDados[0] == 'S');
    fscanf(csv, "%[^,],", vetorParaDados);
    // se tiver comorbidade, variável recebe True, se não recebe False
    vetorCsv[i].comorbidadesPacientes.comorbidadeObesidade = (vetorParaDados[0] == 'S');
    // lendo ficou internado
    fscanf(csv, "%[^\n]%*c", vetorParaDados);
    // se o indivíduo ficou internado, variável recebe True, se não, False
    vetorCsv[i].ficouInternado = (vetorParaDados[0] == 'S');
  }
}
...
void listaCasosEmOrdemAlfabetica(FILE *arquivo, int n, DadosCsv vetorCsv[]) {
  ...
  for (i = 0; i < 79; i++) {
    ...
    total = calculaTotalCasos(municipiosEspiritoSanto[i], vetorCsv);
...
void listaTopNCidades(FILE *arquivo, int n, Data dataInicial, Data dataFinal, DadosCsv vetorCsv[]) {
  ...
  for (i = 0; i < 79; i++) {
    ...
    vetorCasosPorMunicipio[i].numeroDeCasos = calculaQuantidadeDeCasosComDatas(dataInicial, dataFinal, municipiosEspiritoSanto[i], vetorCsv);
...
void imprimePorcentagensItem6(FILE *arquivo, char municipio[], DadosCsv vetorCsv[]) {
  ...
  porcentagemPessoasConfirmadasInternadas(arquivo, municipio, vetorCsv);
  porcentagemPessoasQueMorreram(arquivo, municipio, vetorCsv);
  porcentagemPessoasInternadasQueMorreram(arquivo, municipio, vetorCsv);
}

void imprimeDadosItem7(FILE *arquivo, Data dataInicial, Data dataFinal, DadosCsv vetorCsv[]) {
  // calcualando média entre a dataInicial e a dataFinal
  float media = calculaMediaDeIdadeEntreDatas(dataInicial, dataFinal, vetorCsv);

  // imprimindo dados formatados do arquivo
  fprintf(arquivo, "A media e desvio padrao da idade: %.3f -- %.3f\n", media, fabs(calculaDesvioPradraoEntreDatas(dataInicial, dataFinal, media, vetorCsv)));
  fprintf(arquivo, "A %% de pessoas que morreram sem comorbidade: %.3f%%", calculaPorcentagemPessoasQueMorreramSemComorbidade(dataInicial, dataFinal, vetorCsv));
}
...
Data proximaData(Data *dataInicial) {
  Data proxima;
  proxima.dia = dataInicial->dia;
  proxima.mes = dataInicial->mes;
  proxima.ano = dataInicial->ano;
  ...
  if (proxima.mes == 12 && quantidadeDiasDataInicial == proxima.dia) {
  ...
  else if (proxima.dia == quantidadeDiasDataInicial) {
